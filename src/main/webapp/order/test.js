...
$('.numberoftraveler').on('change', function (e) {
	var number = $(this).val();
	var name = $.trim($(this).attr("productname"));
	var detail = $(".inner_block2").find(`div.${name}`);
	detail.find("p.numberperson").html(number);
	detail.find("p.total").html(`TWD ${detail.find("input.unitprice").val() * number}`);
	detail.find("p.total").attr("total", detail.find("input.unitprice").val() * number);
	var total = 0;
	$("p.total").each(function (index, item) {
		total += parseInt($(item).attr("total"));
		$("#orderpriceamountid").val(total);
	})
	$("span.amount").text(`TWD ${total}`);
